...
class TokenAutoPlugin(Star):
    ...
    def __init__(self, context: Context, config: dict):
        ...
        self.file_type = config.get("file_type", "json").lower()
    ...
    async def export_tokens(self, event: AstrMessageEvent):
        ...
        # 临时文件路径
        temp_file_path = os.path.join(os.path.dirname(self.data_file), f"token_counts.{fmt}")

        try:
            if fmt == "json":
                with open(temp_file_path, "w", encoding="utf-8") as f:
                    json.dump(self.token_counts, f, ensure_ascii=False, indent=4)
            else:
            ...
            chain = [
                comp.File(file=temp_file_path, name=os.path.basename(temp_file_path))
            ]
            yield event.plain_result(f"已按【{fmt}】格式导出。")
            yield event.chain_result(chain)
        except Exception as e:
            logger.error(f"文件导出失败: {e}")
            yield event.plain_result(f"❌ 文件导出失败: {str(e)}")
